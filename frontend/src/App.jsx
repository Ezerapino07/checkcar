...
/* ═══════ API FIELD CONVERTERS ═══════ */
function toAPIVehicle(f){return{titulo:f.titulo,marca:f.marca,modelo:f.modelo,anio:f.anio||null,motor:f.motor,version:f.version,transmision:f.transmision,condicion:f.condicion,kilometros:f.kilometros||null,fecha_ingreso:f.fechaIngreso||null,fecha_venta:f.fechaVenta||null,patente:f.patente,chasis:f.chasis,nro_motor:f.nroMotor,precio_compra:f.precioCompra||null,precio_venta:f.precioVenta||null,precio_minimo:f.precioMinimo||null,descripcion:f.descripcion,anotaciones:f.anotaciones,estado:f.estado||'Disponible',procedencia:f.procedencia,ubicacion:f.ubicacion,estado_cubiertas:f.estadoCubiertas,estado_pintura:f.estadoPintura,estado_motor:f.estadoMotor,estado_interior:f.estadoInterior,vendido:f.vendido||false,vendedor:f.vendedor||null,cliente_venta_id:f.clienteVentaId?Number(f.clienteVentaId):null,fotos:(f.fotos||[]).map(p=>typeof p==='string'?p:p.url).filter(Boolean),gastos:(f.gastos||[]).filter(g=>g.descripcion||g.monto),historial:(f.historial||[]).filter(h=>h.detalle)};}
function fromAPIVehicle(v){return{id:v.id,titulo:v.titulo||'',marca:v.marca||'',modelo:v.modelo||'',anio:v.anio||'',motor:v.motor||'',version:v.version||'',transmision:v.transmision||'Manual',condicion:v.condicion||'Usado',kilometros:v.kilometros||'',fechaIngreso:v.fecha_ingreso?String(v.fecha_ingreso).slice(0,10):'',fechaVenta:v.fecha_venta?String(v.fecha_venta).slice(0,10):'',patente:v.patente||'',chasis:v.chasis||'',nroMotor:v.nro_motor||'',precioCompra:v.precio_compra||'',precioVenta:v.precio_venta||'',precioMinimo:v.precio_minimo||'',descripcion:v.descripcion||'',anotaciones:v.anotaciones||'',estado:v.estado||'Disponible',procedencia:v.procedencia||'Compra directa',ubicacion:v.ubicacion||'Salón principal',estadoCubiertas:v.estado_cubiertas||'Bueno',estadoPintura:v.estado_pintura||'Bueno',estadoMotor:v.estado_motor||'Bueno',estadoInterior:v.estado_interior||'Bueno',vendido:v.vendido||false,vendedor:v.vendedor||'',clienteVentaId:v.cliente_venta_id?String(v.cliente_venta_id):'',fotos:(v.fotos||[]).map(p=>typeof p==='object'&&p!==null?p.url:p).filter(Boolean),gastos:(v.gastos||[]).map(g=>({id:g.id,descripcion:g.descripcion||'',monto:g.monto||''})),historial:(v.historial||[]).map(h=>({id:h.id,fecha:h.fecha?String(h.fecha).slice(0,10):'',detalle:h.detalle||''})),};}
function fromAPIClient(c){return{id:c.id,nombre:c.nombre||'',telefono:c.telefono||'',email:c.email||'',dni:c.dni||'',direccion:c.direccion||'',notas:c.notas||'',vehiculoId:c.vehicle_id?String(c.vehicle_id):''};}
function toAPIClient(f){return{nombre:f.nombre,telefono:f.telefono||null,email:f.email||null,dni:f.dni||null,direccion:f.direccion||null,notas:f.notas||null,vehiculo_id:f.vehiculoId?Number(f.vehiculoId):null};}
...
function VehPage({data,setData,user,allMarcas,addMarca,reloadData}){const [sf,setSf]=useState(false);const [ev,setEv]=useState(null);const [vv,setVv]=useState(null);const [fl,setFl]=useState({search:"",marca:"",estado:"all",sort:"recent",precioMin:"",precioMax:"",kmMin:"",kmMax:"",anioMin:"",anioMax:"",condicion:"",ubicacion:"",transmision:""});const [sfl,setSfl]=useState(false);
  ...
  const save=async f=>{if(hasAPI){try{if(f.id){await apiRequest(`/vehicles/${f.id}`,{method:'PUT',body:JSON.stringify(toAPIVehicle(f))});}else{await apiRequest('/vehicles',{method:'POST',body:JSON.stringify(toAPIVehicle(f))});}await reloadData();}catch(e){alert('Error al guardar vehículo: '+e.message);return;}}else{const nd={...data};const now=new Date().toLocaleString("es-AR");if(f.id){nd.vehicles=data.vehicles.map(v=>v.id===f.id?f:v);nd.activityLog=[{date:now,user:user.name,action:`Editó vehículo: ${f.titulo||f.marca+" "+f.modelo}`},...(data.activityLog||[])];}else{f.id=data.nextVId;nd.nextVId=data.nextVId+1;nd.vehicles=[...data.vehicles,f];nd.activityLog=[{date:now,user:user.name,action:`Agregó vehículo: ${f.titulo||f.marca+" "+f.modelo}`},...(data.activityLog||[])];}setData(nd);}setSf(false);setEv(null);};
  const del=async id=>{if(!confirm("¿Eliminar?"))return;if(hasAPI){try{await apiRequest(`/vehicles/${id}`,{method:'DELETE'});await reloadData();}catch(e){alert('Error al eliminar: '+e.message);}}else{const v=data.vehicles.find(x=>x.id===id);const now=new Date().toLocaleString("es-AR");setData({...data,vehicles:data.vehicles.filter(x=>x.id!==id),activityLog:[{date:now,user:user.name,action:`Eliminó vehículo: ${v?.titulo||v?.marca+" "+v?.modelo}`},...(data.activityLog||[])]});}};
...
/* ═══════ SOLD VEHICLES PAGE ═══════ */
function SoldPage({data,setData,user,reloadData}){
  ...
  return(<div>
    ...
    {sf2&&<VForm vehicle={ev2} allMarcas={[...new Set([...DEF_MARCAS,...(data.customMarcas||[])])].sort()} clients={data.clients||[]} onSave={async f=>{if(hasAPI){try{await apiRequest(`/vehicles/${f.id}`,{method:'PUT',body:JSON.stringify(toAPIVehicle(f))});await reloadData();}catch(e){alert('Error al guardar: '+e.message);return;}}else{const now=new Date().toLocaleString("es-AR");const nd={...data,vehicles:data.vehicles.map(v=>v.id===f.id?f:v),activityLog:[{date:now,user:user.name,action:`Editó vehículo vendido: ${f.titulo||f.marca+" "+f.modelo}`},...(data.activityLog||[])]};setData(nd);}setSf2(false);setEv2(null);}} onCancel={()=>{setSf2(false);setEv2(null);}} onAddMarca={()=>{}}/>}
  </div>);
}
...
function CliPage({data,setData,user,reloadData}){const [sf,setSf]=useState(false);const [ec,setEc]=useState(null);const [q,setQ]=useState("");const filt=data.clients.filter(c=>{if(!q)return true;const s=q.toLowerCase();return[c.nombre,c.telefono,c.email,c.dni].some(f=>(f||"").toLowerCase().includes(s));});const save=async f=>{if(hasAPI){try{if(f.id){await apiRequest(`/clients/${f.id}`,{method:'PUT',body:JSON.stringify(toAPIClient(f))});}else{await apiRequest('/clients',{method:'POST',body:JSON.stringify(toAPIClient(f))});}await reloadData();}catch(e){alert('Error al guardar cliente: '+e.message);return;}}else{const nd={...data};const now=new Date().toLocaleString("es-AR");if(f.id){nd.clients=data.clients.map(c=>c.id===f.id?f:c);nd.activityLog=[{date:now,user:user.name,action:`Editó cliente: ${f.nombre}`},...(data.activityLog||[])];}else{f.id=data.nextCId;nd.nextCId=data.nextCId+1;nd.clients=[...data.clients,f];nd.activityLog=[{date:now,user:user.name,action:`Agregó cliente: ${f.nombre}`},...(data.activityLog||[])];}setData(nd);}setSf(false);setEc(null);};const del=async id=>{if(!confirm("¿Eliminar?"))return;if(hasAPI){try{await apiRequest(`/clients/${id}`,{method:'DELETE'});await reloadData();}catch(e){alert('Error al eliminar: '+e.message);}}else{const c=data.clients.find(x=>x.id===id);const now=new Date().toLocaleString("es-AR");setData({...data,clients:data.clients.filter(x=>x.id!==id),activityLog:[{date:now,user:user.name,action:`Eliminó cliente: ${c?.nombre}`},...(data.activityLog||[])]});}};const gV=id=>data.vehicles.find(v=>v.id===Number(id));
...
export default function App(){
  ...
  const reloadData=useCallback(async()=>{if(!hasAPI)return;try{const[vehs,clis]=await Promise.all([apiRequest('/vehicles'),apiRequest('/clients')]);setDR(prev=>({...prev,vehicles:vehs.map(fromAPIVehicle),clients:clis.map(fromAPIClient)}));}catch{}},[]);
  useEffect(()=>{(async()=>{
    // Try to restore API session
    if(hasAPI){const t=localStorage.getItem('checkcar_token');const u=localStorage.getItem('checkcar_user');if(t&&u){try{const d=await apiRequest('/auth/me');setUser({name:d.user.nombre,role:d.user.rol,username:d.user.email});const[vehs,clis]=await Promise.all([apiRequest('/vehicles'),apiRequest('/clients')]);setDR(prev=>({...prev,vehicles:vehs.map(fromAPIVehicle),clients:clis.map(fromAPIClient)}));}catch{localStorage.removeItem('checkcar_token');localStorage.removeItem('checkcar_user');}}setLoaded(true);return;}
    // Local storage fallback
    const s=await loadS();if(s){if(!s.activityLog)s.activityLog=[];if(!s.customMarcas)s.customMarcas=[];setDR(s);}setLoaded(true);})();},[]);
  const setData=useCallback(nd=>{setDR(nd);if(!hasAPI)saveS(nd);},[]);
  const allMarcas=useMemo(()=>[...new Set([...DEF_MARCAS,...(data.customMarcas||[])])].sort(),[data.customMarcas]);
  const addMarca=useCallback(async m=>{if(!m||allMarcas.includes(m))return;if(hasAPI){try{await apiRequest('/vehicles/stats/marcas',{method:'POST',body:JSON.stringify({nombre:m})});}catch{}}setDR(prev=>({...prev,customMarcas:[...(prev.customMarcas||[]),m]}));},[allMarcas]);
  ...
  return(<div style={{display:"flex",minHeight:"100vh",background:"#f8fafc",fontFamily:"'DM Sans','Segoe UI',system-ui,sans-serif"}}><link href="https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;500;600;700;800&display=swap" rel="stylesheet"/>
    ...
    <main style={{flex:1,padding:"24px 32px",maxWidth:1200,overflowX:"hidden"}}>
      ...
      {page==="vehicles"&&<VehPage data={data} setData={setData} user={user} allMarcas={allMarcas} addMarca={addMarca} reloadData={reloadData}/>}
      {page==="sold"&&<SoldPage data={data} setData={setData} user={user} reloadData={reloadData}/>}
      {page==="sales"&&<SalesPage data={data}/>}
      {page==="clients"&&<CliPage data={data} setData={setData} user={user} reloadData={reloadData}/>}
